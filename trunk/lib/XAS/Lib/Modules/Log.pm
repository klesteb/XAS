...
This module implements a simple light weight logging class. It creates an
internal hash of logging values that is then passed to a mixin for the actual
writing to the log. The currently defined logs are "console", "file", "logstash"
or "syslog". This is implemented as a singleton and is autoloadable when
invoked.
...
standard.
    ...
    loca[l0-7], lpr, mail, news, user, uucp
...
A hashref of values to set the interal logging level with.
...
There is no heirarchy of log levels.
...
varialbe $MESSAGE or in the message stanza in XAS::Class.
